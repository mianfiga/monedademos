    ...
    protected function beforeSave() {
        if (parent::beforeSave()) { //falta comprobar que si no tiene superpermisos que la cuenta sea la del usuario logeado
            ...
            if ($this->isNewRecord) {
                ...
                if ($this->user_password != null || $this->plain_password != null) {   //falta comprobar si la entidad es un usuario
                    if ($this->getScenario() == 'update' && Yii::app()->user->getId() == $this->entity_id
                        ...
                        Yii::app()->user->setFlash('error', Yii::t('error','Pin/Password have not been updated'));
                        return false;
                    }
                }
            }
